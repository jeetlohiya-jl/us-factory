...
def delete_machine(
    ...
    if in_use:
        raise HTTPException(status_code=409, detail="This machine is referenced by an existing RM Consumption or Production record and cannot be deleted. Deactivate it instead.")
    try:
        db.delete(machine)
        db.commit()
    except IntegrityError:
